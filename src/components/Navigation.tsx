...
export function Navigation({ page, onNavigate, onStart, onJourney }: NavigationProps) {
  ...
  return (
    <header className="site-header">
      <div className="nav-wrap">
        ...
        <div className="nav-actions"><button className="journey-link" onClick={onJourney}>My journey <ArrowUpRight size={15} /></button><button className="button button-dark button-small" onClick={onStart}>Start exploring <ArrowRight size={15} /></button></div>
      ...
      {mobileOpen && <nav id="mobile-navigation" className="mobile-nav" aria-label="Mobile navigation"><button onClick={() => navigate('discover')}>Discover <ArrowUpRight size={17} /></button><button onClick={() => navigate('explore')}>Explore <ArrowUpRight size={17} /></button><button onClick={() => navigate('how-it-works')}>How it works <ArrowUpRight size={17} /></button><button onClick={() => { onJourney(); setMobileOpen(false); }}>My journey <ArrowUpRight size={17} /></button><button className="button button-dark" onClick={() => { onStart(); setMobileOpen(false); }}>Start exploring <ArrowRight size={17} /></button></nav>}
    </header>
  );
}